...
scatter(T2,E_values,sz,'r');  xlim([0 tfl]);
...
function E = E_two(t)
    ...
    % Define the duration of the transition between the high and low voltage levels
    transition_duration = 0.001 * t_sw;
    
    % Set the voltage values
    for i = 1:length(t)
        if t(i) < t_sw - transition_duration
            E(i) = E_on;
        elseif t(i) < t_sw
            E(i) = E_on * (t_sw - t(i)) / transition_duration;
        else
            E(i) = 0;
        end
    end
